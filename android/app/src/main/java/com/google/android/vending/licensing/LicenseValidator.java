...
class LicenseValidator {
        ...
        if (responseCode == LICENSED || responseCode == NOT_LICENSED ||
            ...
            try {
                ...
                if (!sig.verify(Base64.decode(signature))) {
                    Log.e(TAG, "Signature verification failed." + signedData);
                    handleInvalidResponse();
                    return;
                }
    ...
    private void handleResponse(int response, ResponseData rawData) {
        ...
        if (mPolicy.allowAccess()) {
            ...
            mCallback.allowWithUserId(response, userId);
        } else {
            mCallback.dontAllow(response);
        }
    }

    private void handleDataResponse(int response, ResponseData rawData) {
        mPolicy.processServerResponse(response, rawData);
    }
